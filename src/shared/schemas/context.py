...
class RequestContext(BaseModel):
    ...
    whatsapp_id: Optional[str] = Field(
        None,
        description="whatsapp phone number for communication identity"
    )
    ...
    primary_language: Optional[str] = Field(
        None,
        description="primary language/dialect (e.g., 'en', 'sw', 'zu', 'fr')"
    )
    
    # connectivity
    network_type: Optional[str] = Field(
        None,
        description="network type: 'edge/2g', 'unstable', 'high-speed'"
    )
    ...
    patient_age: Optional[int] = Field(
        ...
        ge=0,
        le=120,
        description="patient age in years (for dosage calculations)"
    )
    ...
    current_medications: Optional[str] = Field(
        None,
        description="current medications for drug-drug interaction checks"
    )
    allergies: Optional[str] = Field(
        None,
        description="known allergies (e.g., 'Penicillin, Sulfa drugs')"
    )
    ...
    adherence_score: Optional[int] = Field(
        ...
        ge=0,
        le=100,
        description="medication adherence percentage (0-100%)"
    )
    refill_due_date: Optional[str] = Field(
        None,
        description="medication refill due date (ISO format or descriptive)"
    )
    
    # legacy fields (for backward compatibility)
    age: Optional[int] = Field(None, ge=0, le=120, description="age in years (deprecated, use patient_age)")
    gender: Optional[str] = Field(None, description="gender (deprecated, use patient_gender)")
    country: Optional[str] = Field(None, description="ISO country code (e.g., za, ke, us)")
